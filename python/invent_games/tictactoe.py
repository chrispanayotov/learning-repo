...
def isWinner(bo, le):
    """Given a board and a player's letter, this function return True
...
def getComputerMove(board, computerLetter):
    ...
    # First, it checks if there is a win condition in the next move.
    for i in range(1, 10):
    ...
    # Checks if the player could win on their next move and blocks them.
    for i in range(1, 10):
        boardCopy = getBoardCopy(board)
        if isSpaceFree(boardCopy, i):
    ...
    while gameIsPlaying:
        if turn == 'player':
            ...
            if isWinner(theBoard, playerLetter):
                drawBoard(theBoard)
                print('Hooray! You have beaten the computer!')
                gameIsPlaying = False
            else:
            ...
            if isWinner(theBoard, computerLetter):
                drawBoard(theBoard)
                print('The computer has beaten you! You lose! Looser')
                gameIsPlaying = False
            else:
                if isBoardFull(theBoard):
                    drawBoard(theBoard)
